export class BlurryImageLoad {
  supportsCSSFilters(enableWebkit: boolean) {
    ...
    element.style.cssText = (enableWebkit ? '-webkit-' : '') + 'filter: blur(2px)'
    //checking whether the style is computed or ignored
    const test1 = element.style.length != 0
    //checking for false positives of IE
    //I prefer Modernizr's smart method of browser detection
    //@ts-ignore
    const test2 = document.documentMode === undefined || document.documentMode > 9
